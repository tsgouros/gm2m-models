AFRAME.registerComponent('override-catseye-outer', {
  schema: {
  ...
  uniforms: {
    emissionTex: { type: 't' , value: new THREE.TextureLoader().load("../../textures/catseye_OuterNebula.png")},
...
float mapSmoothed(float value, float min1, float max1, float min2, float max2) {
  ...
  void main() {
    ...
    edgeOpacity = smoothstep( 0.3, 0.8, viewAngleFine);
    edgeLightening = mapSmoothed( viewAngleFine, 0.1, 0.4, 0.5, 0.0);
...
float mapSmoothed(float value, float min1, float max1, float min2, float max2) {
  ...
  void main () {
    ...
    vec3 srcEdgeLightened = vec3(1.0) - (vec3(1.0) - srcTexture) * vec3(1.0 - edgeLightening);
    vec3 finalColor = mix(srcTexture, srcEdgeLightened, 0.5);
    float finalAlpha = (srcAlpha + edgeLightening * 3.0) * edgeOpacity;
