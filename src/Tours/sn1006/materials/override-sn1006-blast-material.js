AFRAME.registerComponent('override-sn1006-blast-material', {
  schema: {
    noiseAmt: {type: 'float', default: 0.0},
    viewOpacityThresholds: {type: 'vec2', default: {x: 0.5, y: 0.1} }
  },

  uniforms: {
    noiseAmt: {type: 'float', value: 1.0},
    uTime: {type: 'float', value: 0 },
    emissionTex: { type: 't' , value: new THREE.TextureLoader().load("../../textures/SN1006_Halo_1.png")},
    viewOpacityThresholds: {type: 'vec2', value: {x: 0.5, y: 0.1} }
  },
    ...
    const fragmentShader = `
      // Use low precision.
        ...
        void main () {
          ...
          float opacity = viewAngleOpacity * srcImg.r;
          float colorRamp = smoothstep( 0.0, 0.4, opacity);
          vec3 color = mix( vec3( 0.0, 0.0, 0.75 ), vec3(1.0), colorRamp );
